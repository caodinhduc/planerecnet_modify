...
def get_points_coordinate(depth, intrinsic_inv):
    ...
    xyz = torch.stack((x, y, torch.ones_like(x))).cuda(cuda_id)  # [3, H*W]
...
def get_surface_normal(point_clouds, valid_condition):
    ...
    valid_condition_all = (valid_condition_all > 0.5).cuda(cuda_id)
    ...
    matrix_b = torch.ones([1, H, W, 25, 1]).float().cuda(cuda_id)

    # dot(A.T, A)
    point_multi = torch.matmul(matrix_a_trans, matrix_a_valid)
    matrix_deter = torch.linalg.det(point_multi.to("cpu")).cuda(cuda_id)
    # make inversible
    ...
    # inversible matrix
    inversible_matrix = torch.where(inverse_condition_all, point_multi, diag_matrix.cuda(cuda_id))
    inv_matrix = torch.inverse(inversible_matrix.to("cpu")).cuda(cuda_id)
