...
class VNL_Loss(nn.Module):
    def __init__(self, input_size,
        ...
        self.u0 = torch.tensor(input_size[1] // 2, dtype=torch.float32).cuda(cuda_id) # x, y focal center
        self.v0 = torch.tensor(input_size[0] // 2, dtype=torch.float32).cuda(cuda_id)
    ...
    def init_image_coor(self): # take care of point cloud
        ...
        x = torch.from_numpy(x.copy()).cuda(cuda_id)
        self.u_u0 = x - self.u0
        ...
        y = torch.from_numpy(y.copy()).cuda(cuda_id)
        self.v_v0 = y - self.v0
...
class VNL_Loss_ori(nn.Module):
    ...
    def __init__(self, input_size,
        ...
        #self.fx = torch.tensor([focal_x], dtype=torch.float32).cuda(cuda_id)
        #self.fy = torch.tensor([focal_y], dtype=torch.float32).cuda(cuda_id)
        self.input_size = input_size
        self.u0 = torch.tensor(input_size[1] // 2, dtype=torch.float32).cuda(cuda_id)
        self.v0 = torch.tensor(input_size[0] // 2, dtype=torch.float32).cuda(cuda_id)
        self.init_image_coor()
        self.delta_cos = delta_cos
