    ...
    def forward(self, net, mask_preds, cate_preds, kernel_preds, depth_preds, gt_instances, gt_depths):
        ...
        if cfg.use_lava_loss:
            ...
            if self.dataset_name == 'Stanford 2D3DS':
                ...
                dilate_kernel = torch.autograd.Variable(torch.ones((1, 1, 5, 5)).cuda(cuda_id)) 
...
def compute_gradient_map(depth_map, valid_mask=None):
    ...
    sobel_x = torch.autograd.Variable(sobel_x.cuda(cuda_id))

    sobel_y = torch.Tensor([[1, 2, 1],
                            [0, 0, 0],
                            [-1, -2, -1]])
    sobel_y = sobel_y.view((1, 1, 3, 3))
    sobel_y = torch.autograd.Variable(sobel_y.cuda(cuda_id))
...
class EdgeLoss(nn.Module):
    def __init__(self):
        ...
        self.laplacian_kernel.cuda(cuda_id)
        self.loss = nn.MSELoss().cuda(cuda_id)
    def forward(self, input, target):
        target = target.float()
