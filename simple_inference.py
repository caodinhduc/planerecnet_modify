...
def inference_image(net: PlaneRecNet, path: str, save_path: str = None, depth_mode: str='colored'):
    ...
    frame = torch.from_numpy(frame_np).cuda(cuda_id).float()
...
def ibims1(net: PlaneRecNet, in_folder: str, out_folder: str):
    ...
    for p in sorted(input_list):
        ...
        if rgb is None:
            return
        frame = torch.from_numpy(rgb).cuda(cuda_id).float()
        batch = FastBaseTransform()(frame.unsqueeze(0))
        results = net(batch)
...
def ibims1_pd(net: PlaneRecNet, in_folder: str, out_folder: str):
    ...
    for p in sorted(input_list):
        ...
        if pred_masks is not None:
            ...
            k_matrix = torch.from_numpy(k_matrix).double().cuda(cuda_id)
            intrinsic_inv = torch.inverse(k_matrix).double().cuda(cuda_id)
