...
def parse_args(argv=None):
    ...
    parser.add_argument('--autopsy', dest='autopsy', action='store_true',
                        help='Do not output the status bar. This is useful for when piping to a file.')
    parser.add_argument('--dataset', default='S2D3DStaset', type=str,
    ...
    #                     help='valid annotation file')
    parser.add_argument('--eval_images', default='../scannet/filter_scans/', type=str,
                        help='valid images folder')
...
def evaluate(net: PlaneRecNet, dataset, during_training=False, eval_nums=-1):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            batch = Variable(image.unsqueeze(0)).cuda(cuda_id)
            ...
            gt_masks, gt_boxes, gt_classes, gt_planes, k_matrices = [v.cuda(cuda_id) for k, v in gt_instances.items()]
            pred_masks, pred_boxes, pred_classes, pred_scores, pred_depth = [v for k, v in result.items()]

            gt_depth = gt_depth.cuda(cuda_id)
...
def tensorborad_visual_log(net: PlaneRecNet, dataset, writer: SummaryWriter, iteration, eval_nums):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            frame_tensor = torch.from_numpy(frame_ori).cuda(cuda_id).float()
            batch = Variable(image.unsqueeze(0)).cuda(cuda_id)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        dataset = eval(cfg.dataset.name)(args.eval_images, args.eval_info,transform=BaseTransform(MEANS), has_pos=False)
